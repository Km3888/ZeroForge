...
class EmissionAbsorptionRenderer():
    def __init__(self, param):
        ...
        steps_xy = torch.linspace(-1.0, 1.0, 64)
        self.sample_coords = self.get_grid_coords_3d(steps_z, steps_xy, steps_xy).to(param['device'])
        pass
    ...
    def render(self, volume, axis=2):
        ...
        bs = density.shape[0]
        sample_coords = self.sample_coords.expand(bs, self.sample_size_z, 64, 64, 3)

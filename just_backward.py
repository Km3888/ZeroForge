...
def render_tf(object_voxels):
    ...
    latest_checkpoint = '/tmp/checkpoint/model.ckpt-126650'
    model = models.neural_voxel_renderer_plus_tf2()
    ...
    with tf.GradientTape() as tape:
        tape.watch(a_tf)
        outy = model([a_tf,b_tf,c_tf])
        scalar = tf.reduce_sum(outy)
        dy_dx = tape.gradient(scalar, a_tf)
    return outy
...
def plot_prediction(predictions):
    ...
    plt.savefig('nvr_output_test.png')
    plt.show()
...
def main():
    ...
    predictions = render_tf_backward(voxel,np.ones([1,256,256,3]).astype(np.float32))
